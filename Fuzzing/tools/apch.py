...
def apache_about(urlht, header):
    h = requestUtils.request_header(urlht, header)
    apch = h["Server"]
...
def apache_user_enum(url, header):
    ufounds = []
    l2 = ["~root", "~toor", "~bin", "~daemon", "~adm", "~lp", "~sync", "~shutdown", "~halt", "~mail", "~pop",
          "~postmaster", "~news", "~uucp", "~operator", "~games",
          "~gopher", "~ftp", "~nobody", "~nscd", "~mailnull", "~ident", "~rpc", "~rpcuser", "~xfs", "~gdm", "~apache",
          "~http", "~web", "~www", "~adm", "~admin",
          "~administrator", "~guest", "~firewall", "~fwuser", "~fwadmin", "~fw", "~test", "~user", "~sql", "~data",
          "~database"]
    for i in l2:
        urlht = url + "/" + i
...
def apache_def_enum(url, header):
    founds = []
    l1 = [".htaccess", ".htpasswd", ".meta", ".web", "access_log", "cgi", "cgi-bin", "cgi-pub", "cgi-script", "dummy",
          "error",
          "error_log", "htdocs", "httpd", "httpd.pid", "icons", "logs", "manual", "phf", "printenv", "server-info",
          "server-status",
          "status", "test-cgi", "tmp"]
    for i in l1:
        urlht = url + "/" + i
